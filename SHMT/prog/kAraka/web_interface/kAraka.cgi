...
use Date::Format;
    if (! (-e "$TMPLOC")){
        mkdir "$TMPLOC" or die "Error creating directory $TMPLOC";
    }
    open(TMP1,">>$TMPLOC/parser.log") || die "Can't open $TMPLOC/parser.log for writing";
      if (param) {
      ...
      system("mkdir -p $TMPLOC/tmp_in$pid");
      open(TMP,">$TMPLOC/tmp_in$pid/wor.$pid") || die "Can't open $TMPLOC/tmp_in$pid/wor.$pid for writing";
      print TMP $sentences,"\n";
      close(TMP);
      ...
      system("$myPATH/SHMT/prog/kAraka/call_kAraka_shell.pl $TMPLOC $sentences $encoding $pid $script $sandhi $morph $parse $text_type $mode NOECHO D");
...
print "<script type=\"text/javascript\">\n";
      ...
      print "window.open('http://$CGILOC/options1.cgi?'word='+word+'','popUpWindow','height=500,width=400,left=100,top=100,resizable=yes,scrollbars=yes,toolbar=no,menubar=no,location=no,directories=no, status=yes');\n }\n </script>";
