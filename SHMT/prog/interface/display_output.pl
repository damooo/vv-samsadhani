...
if ( $out_encoding eq "DEV" ){
  ...
  $my_converter_wxHindi="$myPATH/converters/wxHindi-utf8.sh";
      ...
      function show(word,outencoding){\n
      window.open('$CGIPATH/options1.cgi?word='+word+'','popUpWindow','height=500,width=400,left=100,top=100,resizable=yes,scrollbars=yes,toolbar=no,menubar=no,location=no,directories=no, status=yes');\n }\n </script>
      </head>\n
      ...
      open(TMP1,"<$TMP_PATH/tmp_in$pid/wor.$pid") || die "can't open $TMP_PATH/tmp_in$pid/wor.$pid";
      $sent_no = 1;
      $word_no = 1;
      $in = <TMP1>;
      #$in =~ s/।/ ।/g;
      ...
      system("cat $TMP_PATH/in${pid}_trnsltn");
      ...
      <br /> <font color=\"red\">Below we give the step by step analysis of the input. By default only three rows with the original Sanskrit sentence, relevant morph analysis and the Hindi translation are shown. <br/> You can hide/open other rows using the <font color=\"green\">Show/Hide Rows button</font> at the bottom. The color of each cell indicates the vibhakti / category of the word. Details of color coding are available <a target=\"_blank\" href=\"SCLURL/SHMT/anu_help.html#sec1.4\">here</a>. <br /> If you bring the cursor on the sentence ID, you will see the <font color=\"green\">parsed tree (Kaaraka Analysis)</font> for the given sentence. Clicking on this link will show you all the possible parses. <br /> Finally each word is linked to the <font color=\"green\">Apte's dictionary</font>. The link is available along with the morph analysis displayed in row C.</font><br /> <br />";

      system("cat $TMP_PATH/in${pid}.html");
      print "</body></html>";
